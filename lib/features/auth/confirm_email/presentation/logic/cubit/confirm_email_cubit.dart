...
class ConfirmEmailCubit extends Cubit<ConfirmEmailState> {
  ...
  ConfirmEmailCubit(this._repository) : super(const ConfirmEmailState.loading());
  ...
  Future<void> confirmEmail(String email, String token) async {
    ...
    final result = await _repository.confirmEmail(email: email, token: decodedToken);

    result.when(
      success: (ConfirmEmailResponse response) async {
        if (response.isSuccess) {
          //set current user to be verified
